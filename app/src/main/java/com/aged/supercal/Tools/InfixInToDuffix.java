        ...
        for (int i = 0; i < arr.length; i++) {                                    //此处就是上面说的运算过程， 因为list.remove的缘故，所以取出最后一个数个最后两个数  都是size-2
            ...
            switch (arr[i]) {
                ...
                case "/":
                  ...
                  /*  Toast.makeText(InfixInToDuffix.this,  Double.parseDouble(list.remove(size-2))., Toast.LENGTH_SHORT).show();
                   if(Double.parseDouble(list.remove(size-2))==0)  {
                       return "除数不能为0";
                   }
                   */
                    break;
